...
type CustomerClientProps = { customers: CustomerDTO[] };
...
export function CustomerClient({ customers }: CustomerClientProps) {
  ...
  async function handleDelete(c: CustomerDTO) {
    ...
    if (!res.ok) {
      const data = await res.json().catch(() => null);
      alert(data?.error ?? "Could not delete this customer.");
      return;
    }
    router.refresh();
  }
    ...
    <section className="grid gap-5">
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3">
          <h2 className="m-0 text-lg font-semibold text-slate-800">Customers</h2>
              ...
              {customers.length > 0 ? (
                customers.map((c) => (
                  <tr key={c.id} className="border-t border-slate-200">
                    ...
                    <td className="px-4 py-3 text-slate-500">{c.salesCount}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-2">
                        <Button type="button" variant="outline" onClick={() => openEdit(c)}>
                          Edit
                        </Button>
                        <Button
                          type="button"
